...
void main() {
  test('LocaleNotifier toggles supported locales and rejects unsupported ones', () { final notifier = LocaleNotifier(); expect(notifier.state.languageCode, 'fr'); notifier.toggle(); expect(notifier.state.languageCode, 'en'); notifier.setLocale(const Locale('de')); expect(notifier.state.languageCode, 'en'); });
  test('NewsNotifier exposes overridden repository data', () async { final article = Article(title: 'A', url: 'https://example.com'); final container = ProviderContainer(overrides: [newsRepositoryProvider.overrideWithValue(FakeNews(right([article])))]); addTearDown(container.dispose); expect(await container.read(newsNotifierProvider.future), [article]); });
  test('AuthNotifier signs in through an overridden repository', () async { final container = ProviderContainer(overrides: [authRepositoryProvider.overrideWithValue(FakeAuth())]); addTearDown(container.dispose); await container.read(authProvider.future); await container.read(authProvider.notifier).signIn(' user@example.com ', 'pw'); expect(container.read(authProvider).value!.email, 'user@example.com'); });
}
